function [group_data,batch] = sort_data_by_group(batch,exclude,varargin)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
if nargin == 3; n_conds= varargin{1}; end
% load batch data
    ...
    for i_c = 1:n_erps
        ...
        for i_s = 1:length(subjs)
            ...
            if contains(batch.method,'trials')
                subj_data = reshape(subj_data,n_conds,length(subj_data)/n_conds)';
                subj_data = nanmean(subj_data,1);
            end
            % TODO: rescale data
            
            % get subject group
            s_idx  = strcmp(groups.id,subj_id(1:4));
            grp_id = groups.group(s_idx);

%             grp_id = 1;
            if isempty(grp_id) || sum(strcmp(exclude,subj_id)) > 0% || str2num(subj_id(1:4))<1066
                continue
            end
            ...
            %if subj_data(1) > 9 && subj_data(1) <10
             %   disp('9 < subj_data(1) < 10')
            %end
        end
    end
end
...
for i_r = 1:n_rois
    ...
    for i_c = 1:n_erps
        ...
        for i_g = 1:n_grps
            % get group data
            grp = grp_names{i_g};
            if size({group_data.(grp).(roi).(erp){:}},2) == 1
                grp_entries = group_data.(grp).(roi).(erp){:};
            else
